#!/usr/bin/env python3
"""Build the private derived-input bundle for frozen CUHK-X AIM4.

Run from the historical CUHK-X working directory containing the exact frozen
S1/V7 derived artifacts. The official competition Training ZIP is intentionally
excluded and must be attached separately inside Kaggle.
"""
...
ROOT = Path.cwd()
OUT = ROOT / "cuhkx_aim4_private_inputs.zip"
...
print("Checking frozen AIM4 derived inputs...")
manifest = {}
...
with zipfile.ZipFile(
    ...
    for arcname, src in FILES.items():
        if "\\" in arcname:
            raise RuntimeError(f"forbidden Windows archive separator: {arcname}")
