#!/usr/bin/env python3
"""Regression harness for the HF16-earned live-authority substrate.

B64, H24, and HF16 are immutable regression objects. This runner scores exact
certificates but never mutates or teaches from expected labels.
"""
...
def score(cases: list[dict], name: str) -> dict:
    ...
    for case in cases:
        ...
        exp = case["expected"]
        predicted_authorize = got["status"] in AUTHORIZED
        expected_authorize = exp["status"] in AUTHORIZED
        checks = {
            "decision": predicted_authorize == expected_authorize,
        ...
        rows.append({
            ...
            "observed": got,
            "checks": checks,
            "expected_authorize": expected_authorize,
            "predicted_authorize": predicted_authorize,
        })
...
def main() -> None:
    ...
    hf16 = load_packed(HERE / "HF16.json.gz.b64", HF16_SHA)

    result = {
        "object": "SSI_CALC_V0.1/LIVE_AUTHORITY_SUBSTRATE_REGRESSION",
        "kernel_rule_count": len(checker.RULES),
        "rules_added_beyond_R11": 0,
        "frozen_objects": {"B64": 64, "H24": 24, "HF16": 16},
        "B64": score(b64, "B64_CONTRACT"),
        "H24": score(h24, "H24_REGRESSION"),
        "HF16": score(hf16, "HF16_REGRESSION"),
    }
    out = HERE / "LIVE_SUBSTRATE_REGRESSION_RESULT.json"
    out.write_text(json.dumps(result, indent=2) + "\n")

    summary = {
        "kernel_rule_count": result["kernel_rule_count"],
        "rules_added_beyond_R11": 0,
        "B64": result["B64"]["metrics"],
        "H24": result["H24"]["metrics"],
        "HF16": result["HF16"]["metrics"],
    }
    print(json.dumps(summary, indent=2))

    for bucket in ("B64", "H24", "HF16"):
